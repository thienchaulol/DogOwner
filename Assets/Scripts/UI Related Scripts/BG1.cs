...
public class BG1 : MonoBehaviour {
	...
	void display(){
		if (numOfUpgrades == 0f) {	//display initial purchase price for background
			disp.text = BGName + "\n" + "Purchase price: " + purchasePrice;
		}
		else if (numOfUpgrades == 1f) {	//display purchase price for clothes
			disp.text = BGName + "\n" + "Buy clothes: " + purchasePrice;
		}
		else if (numOfUpgrades == 2f) {	//display purchase price for hats
